...
public class UmlsApi {
  ...
  public ImportedMapping importCSV(Reader csvContent, Collection<String> commentColumns)
    ...
    String deduplicationAuthor =
        "SharePoint deduplication"; // to detect comments generated by SharePoint deduplication
    ...
    try {
      ...
      // use review_{author,date,content}_{i} as default if no comment columns are provided
      ...
      for (String[] row = reader.readNext(); row != null; rowIx++, row = reader.readNext()) {
        ...
        for (CommentColumns cc : commentColumnsList) {
          ...
          if (author.equals(deduplicationAuthor)) {
            ...
            String authorContent = String.format("%s: %s", author, content);
            importedMessagesByCode
                .computeIfAbsent(vocId, key -> new HashMap<>())
                .computeIfAbsent(codeId, key -> new LinkedList<>())
                .add(new Message(messageIx++, importAuthor, date, authorContent, true));
          }
        }
      }
      ...
      for (String cui : messagesByConcept.keySet()) {
        ...
        for (String content : messagesByConcept.get(cui)) {
          Message msg = new Message(messageIx++, importAuthor, date, content, true);
      ...
      for (String vocId : importedMessagesByCode.keySet()) {
        ...
        for (String codeId : forVocId.keySet()) {
          ...
          Action created = new Action(importAuthor, date);
          Topic topic = new Topic(topicIx++, "Review imported from codelist", created, null);
      ...
      for (String vocId : deduplicationMessagesByCode.keySet()) {
        ...
        for (String codeId : forVocId.keySet()) {
          ...
          for (String message : messages) {
            topic.messages.add(new Message(messageIx++, importAuthor, date, message, true));
          }
          allTopics
              .byCode
              .computeIfAbsent(vocId, key -> new HashMap<>())
      ...
      for (String vocId : messagesByCode.keySet()) {
        ...
        for (String codeId : forVocId.keySet()) {
          ...
          if (topic == null) {
            Action created = new Action(importAuthor, date);
            topic = new Topic(topicIx++, "Codelist import", created, null);
            topics.put(topic.id, topic);
          }
          for (String message : messages) {
            topic.messages.add(new Message(messageIx++, importAuthor, date, message, false));
          }
        }
      }
