...
public class NonUmlsTargetsTest {
  ...
  @Test
  public void testGetDescendantsLexicographical() throws Exception {
    DataSource connectionPool = CodeMapperApplication.getCodeMapperConnectionPool();
    try (NonUmlsTargets targets = new NonUmlsTargets.Config(connectionPool).createApi()) {
      ...
      String l22Actual =
          descs.get("L22").stream().map(Code::getId).collect(Collectors.joining(", "));
